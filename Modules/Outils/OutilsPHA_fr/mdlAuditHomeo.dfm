inherited frmAuditHomeo: TfrmAuditHomeo
  Left = 1237
  Top = 401
  Caption = 'Ventilation type Hom'#233'o '
  ClientHeight = 403
  ClientWidth = 489
  Constraints.MaxHeight = 437
  Constraints.MaxWidth = 497
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object pCtrlAuditHomeo: TPageControl [0]
    Left = 0
    Top = 0
    Width = 489
    Height = 403
    ActivePage = tShAdHCIP
    Align = alClient
    TabOrder = 0
    OnChange = pCtrlAuditHomeoChange
    object tShAdHCIP: TTabSheet
      Caption = 'Code CIP'
      object pnlAudHCIPGrandPx: TPanel
        Left = 0
        Top = 198
        Width = 481
        Height = 177
        Align = alBottom
        BevelOuter = bvNone
        Caption = 'pnlAudHCIPGrandPx'
        TabOrder = 1
        object grdAHCIPGrandPrix: TPIDBGrid
          Left = 0
          Top = 0
          Width = 481
          Height = 177
          Align = alClient
          BorderStyle = bsNone
          DataSource = dsHGrandPrix
          DefaultDrawing = False
          Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
          MenuColonneActif = False
          StyleBordure = sbAucune
          Options2.LargeurIndicateur = 11
          Options2.LignesParLigneDonnees = 1
          Options2.LignesParTitre = 1
          Options2.CouleurSelection = clHighlight
          Options2.PoliceSelection.Charset = DEFAULT_CHARSET
          Options2.PoliceSelection.Color = clHighlightText
          Options2.PoliceSelection.Height = -11
          Options2.PoliceSelection.Name = 'MS Sans Serif'
          Options2.PoliceSelection.Style = []
          Options2.OptionsImpression.UniqLigneSelectionne = False
          Options2.OptionsImpression.FondCellule = False
          Options2.OptionsImpression.FondTitre = False
          Options2.PointSuspensionDonnees = False
          Options2.PointSuspensionTitre = False
          Options2.CoinsRonds = False
          Details = False
          HauteurEntetes = 17
          Entetes = <
            item
              Alignement = taCenter
              Libelle = 'Hom'#233'opathie "Grand prix"'
              Couleur = $00BF7633
              Police.Charset = DEFAULT_CHARSET
              Police.Color = clWindow
              Police.Height = -11
              Police.Name = 'MS Sans Serif'
              Police.Style = [fsBold]
              Largeur = 450
            end>
          MultiSelection.Active = False
          MultiSelection.Mode = mmsSelection
        end
      end
      object pnlAudHCIPPetitPx: TPanel
        Left = 0
        Top = 0
        Width = 481
        Height = 177
        Align = alTop
        BevelOuter = bvNone
        TabOrder = 0
        object grdAHCIPPetitPrix: TPIDBGrid
          Left = 0
          Top = 0
          Width = 481
          Height = 177
          Align = alClient
          BorderStyle = bsNone
          DataSource = dsHPetitPrix
          DefaultDrawing = False
          Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
          MenuColonneActif = False
          StyleBordure = sbAucune
          Options2.LargeurIndicateur = 11
          Options2.LignesParLigneDonnees = 1
          Options2.LignesParTitre = 1
          Options2.CouleurSelection = clHighlight
          Options2.PoliceSelection.Charset = DEFAULT_CHARSET
          Options2.PoliceSelection.Color = clHighlightText
          Options2.PoliceSelection.Height = -11
          Options2.PoliceSelection.Name = 'MS Sans Serif'
          Options2.PoliceSelection.Style = []
          Options2.OptionsImpression.UniqLigneSelectionne = False
          Options2.OptionsImpression.FondCellule = False
          Options2.OptionsImpression.FondTitre = False
          Options2.PointSuspensionDonnees = False
          Options2.PointSuspensionTitre = False
          Options2.CoinsRonds = False
          Details = False
          HauteurEntetes = 17
          Entetes = <
            item
              Alignement = taCenter
              Libelle = 'Hom'#233'opathie "Petit prix"'
              Couleur = $00BF7633
              Police.Charset = DEFAULT_CHARSET
              Police.Color = clWindow
              Police.Height = -11
              Police.Name = 'MS Sans Serif'
              Police.Style = [fsBold]
              Largeur = 450
            end>
          MultiSelection.Active = False
          MultiSelection.Mode = mmsSelection
        end
      end
    end
    object tShAdHFournisseur: TTabSheet
      Caption = 'Fournisseur'
      ImageIndex = 1
      ExplicitLeft = 0
      ExplicitTop = 0
      ExplicitWidth = 0
      ExplicitHeight = 0
      object pnlAudHFourPetitPx: TPanel
        Left = 0
        Top = 0
        Width = 481
        Height = 177
        Align = alTop
        BevelOuter = bvNone
        TabOrder = 0
        object grdAHFourPetitPrix: TPIDBGrid
          Left = 0
          Top = 0
          Width = 481
          Height = 177
          Align = alClient
          BorderStyle = bsNone
          DataSource = dsHPetitPrix
          DefaultDrawing = False
          Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
          MenuColonneActif = False
          StyleBordure = sbAucune
          Options2.LargeurIndicateur = 11
          Options2.LignesParLigneDonnees = 1
          Options2.LignesParTitre = 1
          Options2.CouleurSelection = clHighlight
          Options2.PoliceSelection.Charset = DEFAULT_CHARSET
          Options2.PoliceSelection.Color = clHighlightText
          Options2.PoliceSelection.Height = -11
          Options2.PoliceSelection.Name = 'MS Sans Serif'
          Options2.PoliceSelection.Style = []
          Options2.OptionsImpression.UniqLigneSelectionne = False
          Options2.OptionsImpression.FondCellule = False
          Options2.OptionsImpression.FondTitre = False
          Options2.PointSuspensionDonnees = False
          Options2.PointSuspensionTitre = False
          Options2.CoinsRonds = False
          Details = False
          HauteurEntetes = 17
          Entetes = <
            item
              Alignement = taCenter
              Libelle = 'Hom'#233'opathie "Petit prix"'
              Couleur = $00BF7633
              Police.Charset = DEFAULT_CHARSET
              Police.Color = clWindow
              Police.Height = -11
              Police.Name = 'MS Sans Serif'
              Police.Style = [fsBold]
              Largeur = 450
            end>
          MultiSelection.Active = False
          MultiSelection.Mode = mmsSelection
        end
      end
      object pnlAutHFourGrandPx: TPanel
        Left = 0
        Top = 198
        Width = 481
        Height = 177
        Align = alBottom
        BevelOuter = bvNone
        TabOrder = 1
        object grdAHFourGrandPrix: TPIDBGrid
          Left = 0
          Top = 0
          Width = 481
          Height = 177
          Align = alClient
          BorderStyle = bsNone
          DataSource = dsHGrandPrix
          DefaultDrawing = False
          Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
          MenuColonneActif = False
          StyleBordure = sbAucune
          Options2.LargeurIndicateur = 11
          Options2.LignesParLigneDonnees = 1
          Options2.LignesParTitre = 1
          Options2.CouleurSelection = clHighlight
          Options2.PoliceSelection.Charset = DEFAULT_CHARSET
          Options2.PoliceSelection.Color = clHighlightText
          Options2.PoliceSelection.Height = -11
          Options2.PoliceSelection.Name = 'MS Sans Serif'
          Options2.PoliceSelection.Style = []
          Options2.OptionsImpression.UniqLigneSelectionne = False
          Options2.OptionsImpression.FondCellule = False
          Options2.OptionsImpression.FondTitre = False
          Options2.PointSuspensionDonnees = False
          Options2.PointSuspensionTitre = False
          Options2.CoinsRonds = False
          Details = False
          HauteurEntetes = 17
          Entetes = <
            item
              Alignement = taCenter
              Libelle = 'Hom'#233'opathie "Grand prix"'
              Couleur = $00BF7633
              Police.Charset = DEFAULT_CHARSET
              Police.Color = clWindow
              Police.Height = -11
              Police.Name = 'MS Sans Serif'
              Police.Style = [fsBold]
              Largeur = 450
            end>
          MultiSelection.Active = False
          MultiSelection.Mode = mmsSelection
        end
      end
    end
    object tShAdHForcage: TTabSheet
      Caption = 'Forcer'
      ImageIndex = 2
      ExplicitLeft = 0
      ExplicitTop = 0
      ExplicitWidth = 0
      ExplicitHeight = 0
      object grdAHForcage: TPIDBGrid
        Left = 0
        Top = 0
        Width = 481
        Height = 375
        Align = alClient
        BorderStyle = bsNone
        DataSource = dsFournisseursDirect
        DefaultDrawing = False
        Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
        PopupMenu = pmnuForcageTypeHomeo
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        MenuColonneActif = False
        StyleBordure = sbAucune
        Options2.LargeurIndicateur = 11
        Options2.LignesParLigneDonnees = 1
        Options2.LignesParTitre = 1
        Options2.CouleurSelection = clHighlight
        Options2.PoliceSelection.Charset = DEFAULT_CHARSET
        Options2.PoliceSelection.Color = clHighlightText
        Options2.PoliceSelection.Height = -11
        Options2.PoliceSelection.Name = 'MS Sans Serif'
        Options2.PoliceSelection.Style = []
        Options2.OptionsImpression.UniqLigneSelectionne = False
        Options2.OptionsImpression.FondCellule = False
        Options2.OptionsImpression.FondTitre = False
        Options2.PointSuspensionDonnees = False
        Options2.PointSuspensionTitre = False
        Options2.CoinsRonds = False
        Details = False
        HauteurEntetes = 17
        Entetes = <>
        MultiSelection.Active = False
        MultiSelection.Mode = mmsSelection
        Columns = <
          item
            Expanded = False
            FieldName = 'LIBELLE'
            Title.Alignment = taCenter
            Width = 352
            Visible = True
            Controle = ccAucun
            OptionsControle.CheckBox.ValeurCoche = '1'
            OptionsControle.CheckBox.ValeurDecoche = '0'
            OptionsControle.ComboBox.ValeurParIndex = False
            OptionsControle.ProgressBar.Couleur = clActiveCaption
            OptionsControle.ProgressBar.Max = 100
            OptionsControle.ProgressBar.Min = 0
            Options = [ocExport, ocImpression]
          end
          item
            Expanded = False
            FieldName = 'NOMBRE_PRODUITS'
            Title.Alignment = taCenter
            Width = 100
            Visible = True
            Controle = ccAucun
            OptionsControle.CheckBox.ValeurCoche = '1'
            OptionsControle.CheckBox.ValeurDecoche = '0'
            OptionsControle.ComboBox.ValeurParIndex = False
            OptionsControle.ProgressBar.Couleur = clActiveCaption
            OptionsControle.ProgressBar.Max = 100
            OptionsControle.ProgressBar.Min = 0
            Options = [ocExport, ocImpression]
          end>
      end
    end
  end
  inherited ilActions: TImageList
    Left = 256
    Top = 120
    Bitmap = {
      
      000000000000}
  end
  object pmnuForcageTypeHomeo: TJvPopupMenu
    Style = msXP
    ImageMargin.Left = 0
    ImageMargin.Top = 0
    ImageMargin.Right = 0
    ImageMargin.Bottom = 0
    ImageSize.Height = 0
    ImageSize.Width = 0
    Left = 148
    Top = 105
    object mnuForcagePetitPx: TMenuItem
      Tag = 2
      Caption = 'Forcer Petit Prix'
      OnClick = mnuForcageTypeHomeoClick
    end
    object mnuForcagePetitGd: TMenuItem
      Tag = 1
      Caption = 'Forcer Grand Prix'
      OnClick = mnuForcageTypeHomeoClick
    end
    object mnuSeparateur_1: TMenuItem
      Caption = '-'
    end
    object mnuForcageAucunPx: TMenuItem
      Caption = 'Forcer Pas hom'#233'o'
      OnClick = mnuForcageTypeHomeoClick
    end
  end
  object dsHGrandPrix: TDataSource
    DataSet = cdsHGrandPrix
    Left = 180
    Top = 366
  end
  object cdsHGrandPrix: TClientDataSet
    Aggregates = <>
    FieldDefs = <>
    IndexDefs = <
      item
        Name = 'cdsHGrandPrixIndex1'
      end>
    Params = <>
    StoreDefs = True
    Left = 180
    Top = 310
  end
  object cdsHPetitPrix: TClientDataSet
    Aggregates = <>
    Params = <>
    Left = 252
    Top = 310
  end
  object dsHPetitPrix: TDataSource
    DataSet = cdsHPetitPrix
    Left = 252
    Top = 366
  end
  object dsFournisseursDirect: TDataSource
    DataSet = dmOutilsPHAPHA_fr.setFournisseursDirect
    Left = 356
    Top = 366
  end
end
