unit mdlTransfertYZYConfiguration;
...
type
  TfrTransfertYZYConfiguration = class(TfrConfiguration)
    grdFusion: TGroupBox;
...
procedure TfrTransfertYZYConfiguration.remplir_combo;
begin
...
constructor TfrTransfertYZYConfiguration.Create(AOwner: TComponent;
  AModule: TModule);
begin
  inherited;
  ...
  begin
    ...
    begin
      if HasAttribute('deconditionnement_stup') then chkStup.State := TCheckBoxState(StrToInt(Attributes['deconditionnement_stup'])) else chkStup.Checked := False;
    end;
...
procedure TfrTransfertYZYConfiguration.Enregistrer;
begin
  inherited;
...
procedure TfrTransfertYZYConfiguration.Initialiser(AOptions: IXMLNode);
begin
  inherited;
