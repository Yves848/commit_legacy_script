...
type
  TdmPHARMONYFRPHA = class(TdmMIPostgreSQLPHA, IRequeteur)
...
var
  dmPHARMONYFRPHA : TdmPHARMONYFRPHA;

implementation

{$R *.dfm}

procedure TdmPHARMONYFRPHA.DataModuleCreate(Sender: TObject);
begin
 inherited;

 dmPHARMONYFRPHA := Self;
 FHistorique := THashedStringList.Create;
end;

procedure TdmPHARMONYFRPHA.DataModuleDestroy(Sender: TObject);
begin
  inherited;
...
function TdmPHARMONYFRPHA.CreerCartefi(
  ...
  with module.Projet.FichierProjet.DocumentElement.ChildNodes['modules'].ChildNodes['import'].ChildNodes['options'].ChildNodes['carte_fidelite'] do
    ...
    begin
      ...
      begin
        ...
        Result := ExecuterPS('Programme fidelite', 'PS_PHARMONYFR_CREER_CF2',
...
procedure TdmPHARMONYFRPHA.SupprimerDonnees(
...
function TdmPHARMONYFRPHA.CreerDocument(AIDClient: Integer;
  ALibelle : string; AFichier: string): TResultatCreationDonnees;
begin
  Result := ExecuterPS('SCANS', 'PS_PHARMONYFR_CREER_DOCUMENT', VarArrayOf([AIDClient, ALibelle, AFichier]));
end;
